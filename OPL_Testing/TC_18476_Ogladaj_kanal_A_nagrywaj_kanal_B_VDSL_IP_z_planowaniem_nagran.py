...
class TC_18476_Ogladaj_kanal_A_nagrywaj_kanal_B_VDSL_IP_z_planowaniem_nagran(TC_OPL_template):
    '''
    ...
    def test(self):
        try:
            ...
            if Env.ZONE != "IPTV": 
                self.fail("   ERR   test can only run on IPTV")
            ...
            self.assertTrue(self.page.zapToChannel(2), "   ERR   Can't zap to channel 34")
            for i in range(5):
                if(self.page.checkLive(False)):
