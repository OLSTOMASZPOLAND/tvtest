...
class TC_18527_T999999_032(TC_OPL_template):
    '''Implementation of the HP QC test ID - 18527 - T999999 _W_and_R_bez_TS_najpierw_live_na_kanale_1_instant_na_kanale_2
    ...
    def test(self):
        try:
            ...
            ''' step '''
            self.logStepBeginning("instant recording on channel {}, live on channel {}".format(self.rc.getChannelTVPPolonia, self.rc.getChannelBBCHD))
            ...
            self.assertFalse(self.page.zapToChannel(self.rc.getChannelTVPPolonia), "   ERR   cannot zap to {}".format(self.rc.getChannelTVPPolonia))

            self.assertFalse(self.page.checkLive(), "   ERR   channel displays correctly")

            self.logStepResults("instant recording on channel {}, live on channel {}".format(self.rc.getChannelTVPPolonia, self.rc.getChannelBBCHD))
