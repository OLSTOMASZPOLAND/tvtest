...
class TC_3343_T014474_PVR (TC_OPL_template):
    """Implementation of the HP QC test ID - 3343 - T014474_Set payment-parental control to activated with level equal -16 All public - steps regarding PVR
    ...
    def test(self):
        try:
            ...
            ''' step '''
            self.logStepBeginning("STEP - watch PVR CSA5")
            if(not self.page.goToPvrByCsaCategory(ParentalControl.CssClassCsa5, inAdults = True)):
                self.assertTrue(False, InfoMessages.CsaContentNotFound)
            self.assertTrue(self.page.playPvrThenBackToPvrScreen())
            self.logStepResults("STEP - watch PVR CSA5")
