...
class TC_3346_T014477_LIVE (TC_OPL_template):
    """Implementation of the HP QC test ID - 3346 - T014477_Set payment-parental control to activated with level equal -7 - steps regarding LIVE
    ...
    def test(self):
        try:
            ...
            ''' step '''
            self.logStepBeginning("STEP - watch LIVE CSA1")
            if(not self.page.goToEpgProgramPlayedNowByCsaCategory(ParentalControl.CssClassCsa1)):
                self.assertTrue(False, InfoMessages.CsaContentNotFound)
            self.assertTrue(self.page.playEpgProgram())
            self.logStepResults("STEP - watch LIVE CSA1")
