...
class TC_3410_T015216_Deactive_tracking_from_my_account_set_opl_out_parameter(TC_OPL_template):
    """
    Implementation of the HP QC test ID TC 3410 T015216_Deactive_tracking_from_my_account_set_opl_out_parameter.
    ...
    def test(self):
        try:
            ...
            ''' step '''
            self.logStepBeginning("STEP 1- Set activate Recommendation ")
            self.assertTrue(self.page.setRecommendation('deactivate'),'ERR in set deactivate Recommendation')
            self.logStepResults("STEP 1- Set activate Recommendation ")
            ...
            if find==None:
                time.sleep(1)
                print 'Don`t finding icon - deactivated tracking'
            elif find==True: 
                time.sleep(1)
