...
class TC_3420_T015654_Reload_prepaid_account_payment_control_activated(TC_OPL_template):
    '''
    Coverage: HP QC test ID - 3420 - T015654_Reload prepaid account - payment control activated
    ...
    def test(self):
        ...
        try:
            ...
            ''' Step 4 '''
            self.logStepBeginning("STEP 4 - Enter the correct confidential code and validate")
            
            self.rc.sendNumberSequence(Env.PARENTAL_CODE)
            self.rc.sendKeys(["KEY_OK"])
            time.sleep(2)
            
            self.logStepResults("STEP 4 - Enter the correct confidential code and validate")
            
            ''' Step 5 '''
            self.logStepBeginning("STEP 5 - For each choice of reloading account check that")
            ...
            self.logStepResults("STEP 5 - For each choice of reloading account check that")
            
            ''' Step 6 '''
            self.logStepBeginning("STEP 6 - Choose amount to be credited and validate - 20zl")
            ...
            self.logStepResults("STEP 6 - Choose amount to be credited and validate - 20zl")
