...
class TC_3676_T015933_add_bookmark_on_vod_when_vps_reached_from_vod_catalog(TC_OPL_template):
    '''Implementation of the HP QC test ID - 3676 -_add_bookmark_on_vod_when_vps_reached_from_vod_catalog
    ...
    def test(self):
        try:
            ...
            if not vod:
                self.fail("   ERR   cannot get info from vod page")
            ...
            self.logStepBeginning("check if bookmarked item displays in my selection")
            ...
            self.logStepResults("check if bookmarked item displays in my selection")
